...
int main(int argc, char* argv[])
{
	...
	// 1) Print an immediate console message:
	printf("Initializing HID system.\n");
	printf("Running.Please wait...\n");


	/* 2) Redirect stdout to HID-Output.txt */
	FILE* fp = freopen("HID-Output.txt", "w", stdout);
	if (!fp) {
		/* If we cannot open the file, print an error to stderr and exit */
		fprintf(stderr, "Error: cannot open HID-Output.txt for writing.\n");
		return 1;
	}

	/* From here onward, every printf call writes to HID-Output.txt instead of the console */

	printf("Writing all HID info to HID-Output.txt ...\n\n");
	...
	if (!handle) {
		...
		// Now reopen the console for stdout
	#ifdef _WIN32
		...
		// 4) Print a simple success message to the console (not in the file!)
		printf("All HID output was written to 'HID-Output.txt'.\n");
	...
	// 4) Print a simple success message to the console (not in the file!)
	printf("All HID output was written to 'HID-Output.txt'.\n");
	printf("Success! Please close the program.\n");
	return 0;
}
